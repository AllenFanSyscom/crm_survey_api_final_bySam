...
{
    [Authorize]
    [Route("api/Survey/Question/Film")]
    [ApiController]
    public class SurveyQuestionFilmController : ControllerBase
    {
        /// <summary>
        /// 問卷系統後臺--設計問卷--影片題型
        /// </summary>
        ...
        //private readonly ILogger<SurveyQuestionFilmController> _logger;  //可實現log在console輸出
        private DBHelper _db;
        //public SurveyQuestionFilmController(ILogger<SurveyQuestionFilmController> logger)
        public SurveyQuestionFilmController()
        {
            //_logger = logger;
            //_db = new DBHelper();
            //_logger = logger;
        ...
        //    List<SurveyQuestionFilm> lstQDetail = new List<SurveyQuestionFilm>();
        ...
        public String Insert([FromBody] Object value)  //暫時以Body Json的形式傳遞參數
        {
            ...
            try
            {
                ...
                if (jo["SurveyId"] == null||String.IsNullOrWhiteSpace(jo["SurveyId"].ToString()))
                {
                    ...
                    replyData.data = "";
                    Log.Error("影片題型-新增失敗!" + "參數SurveyId不能為空！");
                    return JsonConvert.SerializeObject(replyData);
                }
            ...
            var QuestionId = Guid.NewGuid().ToString();
            var QuestionSeq = 0; //這個需要自動累加
            var QuestionType = jo["QuestionType"] == null ? 12 : Convert.ToInt32(jo["QuestionType"]);
            ...
            //-------sql para----end
                ...
                catch (Exception ex)
                {
                    //新增成功，查詢失敗
                    replyData.data = "";
                    Log.Debug("影片題型新增成功,查詢返回結果失敗!" + ex.Message);
                }
            }
            catch (Exception ex)
            {
        ...
        public String Update([FromBody] Object value)
        {
            ...
            try
            {
                ...
                if (jo["SurveyId"] == null)
                {
                    ...
                    replyData.data = "";
                    Log.Error("影片題型-編輯失敗!" + "參數SurveyId不能為空！");
                    return JsonConvert.SerializeObject(replyData);
                }
            var SurveyId = jo["SurveyId"].ToString();
            //SurveyId 必須有?
            if (jo["QuestionId"] == null)
            {
                ...
                replyData.data = "";
                Log.Error("影片題型-編輯失敗!" + "QuestionId不能為空！");
                return JsonConvert.SerializeObject(replyData);
            }
           ...
           // var UpdUserId = "00000000-0000-0000-0000-000000000000";
            ...
            var obj2 = new SqlParameter("@UpdUserId", SqlDbType.UniqueIdentifier);
            obj2.Value = UpdUserId.ValidGuid();
            sqlParams.Add(obj2);
            ...
            Log.Debug("影片題型-編輯:" + sSql);
                ...
                if (iR < 1)
                {
                    ...
                    Log.Debug($"影片題型：問卷{SurveyId}題型{QuestionId}不存在！");
                }
                else
                {
                    replyData.code = "200";
                ...
                catch (Exception ex)
                {
                    //新增成功，查詢失敗
                    replyData.data = "";
                    Log.Debug("影片題型-編輯成功,查詢返回結果失敗!" + ex.Message);
                }
            }
            catch (Exception ex)
            {
                ...
                replyData.data = "";
                Log.Error("影片題型-編輯記錄失敗!" + ex.Message);
            }
            //返回
            return JsonConvert.SerializeObject(replyData);
        ...
        public String Delete(String surveyId, String questionId)
        {
            ...
            if (surveyId == null || questionId == null)
            {
                ...
                replyData.data = "";
                Log.Error("設計問卷-影片--刪除失敗!" + "參數surveyId、questionId未傳入！");
                return JsonConvert.SerializeObject(replyData);
            }
            ...
            try
            {
                ...
                replyData.data = iR;
                Log.Debug("影片題型-刪除記錄完成。共刪除{iR}筆。");
                //return Ok(iR);
            }
            catch (Exception ex)
            {
                ...
                replyData.data = "";
                Log.Error("影片題型-刪除失敗!" + ex.Message);
            }
            //return Ok(replyData);
            return JsonConvert.SerializeObject(replyData);
        }
    ...
    public class SurveyQuestionFilm
    {
