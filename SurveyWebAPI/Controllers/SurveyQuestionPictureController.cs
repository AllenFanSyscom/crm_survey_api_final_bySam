...
{
    [Authorize]
    [Route("api/Survey/Question/Picture")]
    [ApiController]
    public class SurveyQuestionPictureController : ControllerBase
    {
        /// <summary>
        /// 問卷系統後臺--設計問卷--圖片題型
        /// </summary>
        ...
        //private readonly ILogger<SurveyQuestionPictureController> _logger;  //可實現log在console輸出
        private DBHelper _db;
        //public SurveyQuestionPictureController(ILogger<SurveyQuestionPictureController> logger)
        public SurveyQuestionPictureController()
        {
            //_logger = logger;
            //_db = new DBHelper();
            //_logger = logger;
        ...
        //    List<SurveyQuestionPicture> lstQDetail = new List<SurveyQuestionPicture>();
        ...
        public String Insert([FromBody] Object value)  //暫時以Body Json的形式傳遞參數
        {
            ...
            var QuestionType = jo["QuestionType"] == null ? 11 : Convert.ToInt32(jo["QuestionType"]);
            ...
            sqlParams[22].Value = BlankMaxLimit.ValidInt16Bit();
            sqlParams[23].Value = BlankMinLimit.ValidInt16Bit();
            ...
            //-------sql para----end
                ...
                catch (Exception ex)
                {
                    //新增成功，查詢失敗
                    replyData.data = "";
                    Log.Debug("圖片題型新增成功,查詢返回結果失敗!" + ex.Message);
                }
            }
            catch (Exception ex)
            {
        ...
        public String Update([FromBody] Object value)
        {
            ...
            if (jo["QuestionId"] == null)
            {
                ...
                replyData.data = "";
                Log.Error("圖片題型-編輯失敗!" + "QuestionId不能為空！");
                return JsonConvert.SerializeObject(replyData);
            }
            ...
            Log.Debug("圖片題型-編輯:" + sSql);
                ...
                if (iR < 1)
                {
                    ...
                    //replyData.code = "-1";
                    //replyData.message = $"無此筆問卷題型資料。";
                    Log.Debug($"圖片題型：問卷{SurveyId}題型{QuestionId}不存在！");
                }
                else
                {
                    replyData.code = "200";
                ...
                try
                {
                    ...
                    sqlParamsA[0].Value = SurveyId.ValidGuid();
                    sqlParamsA[1].Value = SurveyId.ValidGuid();
                    //-------sql para----end
                    var result = ExecuteQuery($"SELECT * FROM QUE002_QuestionnaireDetail WHERE SurveyId=@SurveyId AND QuestionId=@QuestionId' ", sqlParamsA);
                    if (result.Count == 1)
                        replyData.data = result[0];  //應該只有一筆
                ...
                catch (Exception ex)
                {
                    //新增成功，查詢失敗
                    replyData.data = "";
                    Log.Debug("圖片題型-編輯成功,查詢返回結果失敗!" + ex.Message);
                }
            }
            catch (Exception ex)
            {
                ...
                replyData.data = "";
                Log.Error("圖片題型-編輯記錄失敗!" + ex.Message);
            }
            //返回
            return JsonConvert.SerializeObject(replyData);
        ...
        public String Delete(String surveyId, String questionId)
        {
            ...
            if (surveyId == null || questionId == null)
            {
                ...
                replyData.data = "";
                Log.Error("設計問卷-圖片--刪除失敗!" + "參數surveyId、questionId未傳入！");
                return JsonConvert.SerializeObject(replyData);
            }
            ...
            try
            {
                ...
                replyData.data = iR;
                Log.Debug("圖片題型-刪除記錄完成。共刪除{iR}筆。");
                //return Ok(iR);
            }
            catch (Exception ex)
            {
                ...
                replyData.data = "";
                Log.Error("圖片題型-刪除失敗!" + ex.Message);
            }
            //return Ok(replyData);
            return JsonConvert.SerializeObject(replyData);
        }
    ...
    public class SurveyQuestionPicture
    {
