...
{
    [Authorize]
    [Route("api/Survey/Question/BaseInfo")]
    [ApiController]
    public class SurveyQuestionBaseInfoController : BaseController
    {
        /// <summary>
        /// 問卷系統後臺--設計問卷--基本資料
        /// </summary>
        /// <designer>Allen Fan/Gem He</design>
        /// <History> 1. 2020/08/17 Gem Create
        /// </History>


        //private readonly ILogger<SurveyQuestionBaseInfoController> _logger;  //可實現log在console輸出
        private DBHelper _db;
        //public SurveyQuestionBaseInfoController(ILogger<SurveyQuestionBaseInfoController> logger)
        public SurveyQuestionBaseInfoController()
        {
            //_logger = logger;
            //_db = new DBHelper();
            //_logger = logger;
        ...
        {
            Log.Debug("設計問卷-基本資料-單題查詢...");

            //List<SurveyQuestionBaseInfo> lstQDetail = new List<SurveyQuestionBaseInfo>();
            ...
            if (QuestionId != null)
            {
                ...
                var obj = new SqlParameter("@QuestionId", SqlDbType.UniqueIdentifier);
                obj.Value = QuestionId.ValidGuid();
                sqlParams.Add(obj);
            ...
            try
            {
                lstQDetail = ExecuteQuery(sSql, sqlParams);
                replyData.code = "200";
        ...
        public String Insert([FromBody] Object value)
        {
            ...
            SetValidRepoBy( actId, (JObject)JsonConvert.DeserializeObject(value.ToString()) );
            var jo = GetValidFromRepo<JObject>( actId );

            //var jo = (JObject) JsonConvert.DeserializeObject( value.ToString() );
            ...
            if (jo["SurveyId"] == null || String.IsNullOrWhiteSpace(jo["SurveyId"].ToString()))
            {
                ...
                //報告錯誤
                replyData.code = "-1";
                replyData.message = $"單題新增失敗！參數SurveyId不能為空！";
                replyData.data = "";
                Log.Error("單題新增失敗!" + "參數SurveyId不能為空！");
                return JsonConvert.SerializeObject(replyData);
            }
            //獲取操作員資訊
	        ...
	        var QuestionType=jo["QuestionType"] == null ? 1 : Convert.ToInt32(jo["QuestionType"]);
            ...
            sqlParams[27].Value = UpdUserId.ValidGuid();
                ...
                Log.Debug("INSERT INTO QUE002_QuestionnaireDetail:" + sSql);
                bool authUserId = SurveyWebAPI.Utility.Common.checkUserId(UpdUserId); //先檢查
                if(authUserId){
                ...
                try
                {
                    ...
                    var result = ExecuteQuery($"SELECT * FROM QUE002_QuestionnaireDetail WHERE SurveyId=@SurveyId AND QuestionId=@QuestionId ", sql1Params);
                    if(result.Count==1)
                ...
                catch (Exception ex)
                {
                    //新增成功，查詢失敗
                    replyData.data = "";
                    Log.Debug("單題新增成功,查詢返回結果失敗!" + ex.Message);
                }
            }
            catch (Exception ex)
            {
        ...
        public String Update([FromBody] Object value)
        {
            ...
            if (jo["QuestionId"] == null )
            {
                //報告錯誤
                replyData.code = "-1";
                replyData.message = $"儲存失敗！QuestionId不能為空！";
                replyData.data = "";
                Log.Error("儲存失敗!" + "QuestionId不能為空！");
                return JsonConvert.SerializeObject(replyData);
            }
            ...
            obj1 = new SqlParameter("@QuestionId", SqlDbType.UniqueIdentifier);
            obj1.Value = QuestionId.ValidGuid();
            sqlParams.Add(obj1);
            ...
            if (jo["BlankValidType"] != null)
            {
                ...
                var obj = new SqlParameter("@BlankDefaultWords", SqlDbType.Int);
           ...
           if (jo["QuestionImage"] != null)
            ...
            if (jo["MultiOptionLimit"] != null)
            {
                ...
                sSql += $" MultiOptionLimit=@MultiOptionLimit,";
            ...
            Log.Debug("基本資料-單題編輯儲存:" + sSql);
                ...
                catch (Exception ex)
                {
                    //新增成功，查詢失敗
                    replyData.data = "";
                    Log.Debug("單題編輯成功,查詢返回結果失敗!" + ex.Message);
                }
            }
            catch (Exception ex)
            {
        ...
        public String Delete(String surveyId, String questionId)
        {
            ...
            if(surveyId==null||questionId==null)
            {
                ...
                replyData.data = "";
                Log.Error("刪除失敗！參數surveyId、questionId未傳入！");
                return JsonConvert.SerializeObject(replyData);
            }
            string SurveyId = surveyId;// jo["SurveyId"].ToString();
            string QuestionId = questionId;// jo["QuestionId"].ToString();

            string sSql1 = string.Format("DELETE FROM QUE002_QuestionnaireDetail WHERE SurveyId=@SurveyId AND QuestionId=@QuestionId ");
            ...
            string sSql2 = string.Format("DELETE FROM QUE003_QuestionnaireOptions WHERE QuestionId=@QuestionId ");
            //-------sql para----start
            SqlParameter[] sql1Params = new SqlParameter[] {
                      new SqlParameter("@QuestionId", SqlDbType.UniqueIdentifier),

                };
            sql1Params[0].Value = QuestionId.ValidGuid();

            //-------sql para----end

            var list = new List<KeyValuePair<string, SqlParameter[]>>();
            list.Add(new KeyValuePair<string, SqlParameter[]>(sSql1, sqlParams));
            list.Add(new KeyValuePair<string, SqlParameter[]>(sSql2, sql1Params));
            try
            {
                int iR = _db.ExecuteSqlTran(list);
                replyData.code = "000";
                replyData.message = $"刪除單題完成。";
                replyData.data = iR;
                Log.Debug("刪除單題完成。共刪除{iR}筆。");
            ...
            {
                replyData.code = "-1";
                replyData.message = $"刪除單題失敗！{ex.Message}.";
                replyData.data ="";
                Log.Error("刪除單題資料失敗!" + ex.Message);
            }
            //return Ok(replyData);
            return JsonConvert.SerializeObject(replyData);
        }
        ...
        /// <param name="sSql"></param>
        /// <returns></returns>
        private List<QUE002_QuestionnaireDetail> ExecuteQuery(String sSql, List<SqlParameter> cmdParams)
        {
            List<QUE002_QuestionnaireDetail> lstQDetail = new List<QUE002_QuestionnaireDetail>();
            try
            {
    ...
    public class SurveyQuestionBaseInfo
    {
